...
async fn get_validators(url: String) -> Result<Vec<ValidatorMetadata>> {
    ...
    let request = r#"{"jsonrpc":"2.0", "method":"sui_getValidators", "id":1}"#;
    let response = client
        .post(url)
            ...
            match get_validators(url.clone()).await {
                Ok(peers) => {
                    ...
                    let mut allowed = allowlist
                        .write()
                        // TODO is expect ok here? write can panic in some cases per docs
                        .expect("unable to obtain rwlock for peer list update");
                    // empty our list, may result in an empty allow list if len(peers) == 0
                    allowed.clear();
                    for v in peers {
                        ...
                        match Ed25519PublicKey::from_bytes(&v.network_pubkey_bytes) {
                            Ok(client_public_key) => {
                                ...
                                allowed.insert(client_public_key);
                            }
                            Err(error) => {
                                error!(
